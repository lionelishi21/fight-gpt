...
exports.migrateCharacterEncyclopedia = migrateCharacterEncyclopedia;
exports.rollbackMigration = rollbackMigration;
exports.convertSystemMechanicsToGameRules = convertSystemMechanicsToGameRules;
 ...
 */
function convertSystemMechanicsToGameRules(systemMechanics) {
 ...
 */
async function createBackup(documents) {
    ...
    const backupData = documents.map((doc) => ({
        ...
        original_system_mechanics: doc.system_mechanics,
    }));
    fs.writeFileSync(backupFile, JSON.stringify(backupData, null, 2));
    logger_1.Logger.info(`✅ Backup created: ${backupFile}`);
    return backupFile;
}
/**
 * Migrate CharacterEncyclopedia documents
 */
async function migrateCharacterEncyclopedia(dryRun = false) {
    try {
        logger_1.Logger.info('Starting Character Encyclopedia migration...');
        if (dryRun) {
        ...
        logger_1.Logger.info(`Found ${documents.length} CharacterEncyclopedia documents`);
        if (documents.length === 0) {
            logger_1.Logger.warn('No documents found to migrate');
            await database_1.Database.disconnect();
            return { migrated: 0, skipped: 0, errors: 0 };
        }
        // Create backup
        const backupFile = await createBackup(documents);
        logger_1.Logger.info(`Backup saved to: ${backupFile}`);
        let migrated = 0;
        let skipped = 0;
        let errors = 0;
        for (const doc of documents) {
            try {
                ...
                if (!docAny.system_mechanics || !Array.isArray(docAny.system_mechanics)) {
                    logger_1.Logger.warn(`Document ${doc._id} (${doc.game_id}/${doc.character_id}) has no system_mechanics to migrate. Skipping.`);
                    skipped++;
                    continue;
                }
                // Check if document already has game_rules
                if (doc.game_rules && doc.game_rules.length > 0) {
                    logger_1.Logger.warn(`Document ${doc._id} (${doc.game_id}/${doc.character_id}) already has game_rules. Skipping.`);
                    skipped++;
                    continue;
                }
                ...
                if (dryRun) {
                    logger_1.Logger.info(`[DRY RUN] Would migrate ${doc.game_id}/${doc.character_id}:`);
                    logger_1.Logger.info(`  - system_mechanics: ${docAny.system_mechanics.length} items`);
                    logger_1.Logger.info(`  - game_rules: ${gameRules.length} items`);
                    migrated++;
                }
                else {
                    // Update document
                    ...
                    await doc.save();
                    logger_1.Logger.info(`✅ Migrated ${doc.game_id}/${doc.character_id}:`);
                    logger_1.Logger.info(`  - Converted ${docAny.system_mechanics.length} system_mechanics to ${gameRules.length} game_rules`);
                    migrated++;
                }
            }
            catch (error) {
                logger_1.Logger.error(`Failed to migrate document ${doc._id}:`, error);
                errors++;
            }
        }
        ...
        if (dryRun) {
            ...
            logger_1.Logger.info(`   Run without --dry-run to apply changes.`);
        }
        else {
...
async function rollbackMigration(backupFile) {
    try {
        logger_1.Logger.info(`Starting rollback from backup: ${backupFile}`);
        ...
        logger_1.Logger.info(`Found ${backupData.length} documents in backup`);
        let restored = 0;
        let errors = 0;
        for (const backupDoc of backupData) {
            try {
                const doc = await CharacterEncyclopedia_1.CharacterEncyclopedia.findById(backupDoc._id);
                if (!doc) {
                    logger_1.Logger.warn(`Document ${backupDoc._id} not found. Skipping.`);
                    continue;
                }
                ...
                if (backupDoc.original_system_mechanics) {
                    doc.system_mechanics = backupDoc.original_system_mechanics;
                }
                await doc.save();
                logger_1.Logger.info(`✅ Restored ${backupDoc.game_id}/${backupDoc.character_id}`);
                restored++;
            }
            catch (error) {
                logger_1.Logger.error(`Failed to restore document ${backupDoc._id}:`, error);
                errors++;
            }
        }
        logger_1.Logger.info(`\n✅ Rollback completed!`);
        logger_1.Logger.info(`   Restored: ${restored}`);
        logger_1.Logger.info(`   Errors: ${errors}`);
        // Disconnect from database
        await database_1.Database.disconnect();
        logger_1.Logger.info('Database disconnected');
        ...
        logger_1.Logger.error('Rollback error:', error);
        await database_1.Database.disconnect();
        throw error;
    }
}
...
if (require.main === module) {
    ...
    if (command === 'rollback' && args[1]) {
        rollbackMigration(args[1])
            .then(() => {
            logger_1.Logger.info('Rollback finished');
            process.exit(0);
        })
            .catch((error) => {
            logger_1.Logger.error('Rollback error:', error);
        ...
        migrateCharacterEncyclopedia(dryRun)
            .then(() => {
            logger_1.Logger.info('Migration finished');
            process.exit(0);
        })
            .catch((error) => {
            logger_1.Logger.error('Migration error:', error);
...
//# sourceMappingURL=migrateCharacterEncyclopedia.js.map
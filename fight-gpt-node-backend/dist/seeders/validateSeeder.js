...
const seedCharacterEncyclopedia_1 = require("./seedCharacterEncyclopedia");
...
function testMoveConversion() {
    ...
    for (const testMove of testMoves) {
        ...
        try {
            const converted = (0, seedCharacterEncyclopedia_1.convertMoveToEncyclopediaFormat)(mockMove, 'Ryu');
            // Validate structure
            ...
            const hasFrameData = converted.frame_data &&
...
function testInputNotation() {
    ...
    const testCases = [
        { name: 'Hadoken', category: 'special', expected: '236P' },
        { name: 'Shoryuken', category: 'special', expected: '623P' },
        { name: 'Tatsumaki Senpukyaku', category: 'special', expected: '214K' },
        { name: 'Denjin Hadoken', category: 'super', expected: '236236P' },
        { name: 'Unknown Move', category: 'special', expected: '236P' }, // Fallback
    ];
    let passed = 0;
    let failed = 0;
    for (const testCase of testCases) {
        try {
            const input = (0, seedCharacterEncyclopedia_1.generateInputNotation)(testCase.name, 'Ryu', testCase.category);
...
function testHowToPerform() {
    ...
    for (const testCase of testCases) {
        try {
            const description = (0, seedCharacterEncyclopedia_1.generateHowToPerform)('Test Move', 'Ryu', 'special');
...
function testGameRules() {
    ...
    for (const characterName of testCharacters) {
        try {
            const rules = (0, seedCharacterEncyclopedia_1.getSF6GameRules)(characterName);
            // Validate structure
...
async function testCharacterConversion() {
    ...
    try {
        const converted = await (0, seedCharacterEncyclopedia_1.convertCharacterToEncyclopedia)(mockCharacter);
        if (!converted) {
...
//# sourceMappingURL=validateSeeder.js.map
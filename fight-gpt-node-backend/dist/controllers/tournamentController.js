...
exports.getUpcomingTournaments = void 0;
...
const getUpcomingTournaments = async (req, res) => {
    ...
    `;
        ...
        const mappedData = json.data?.tournaments?.nodes?.map((t) => {
            const mainEvent = t.events?.[0];
            const gameName = mainEvent?.videogame?.name || "MULTI-TITLE";
            let color = "text-white";
            if (gameName.toLowerCase().includes("street fighter"))
                color = "text-primary";
            else if (gameName.toLowerCase().includes("tekken"))
                color = "text-secondary";
            else if (gameName.toLowerCase().includes("guilty gear"))
                color = "text-neon-green";
            return {
                id: t.id,
                title: t.name,

...
class MissionService {
     ...
     */
    static async verifyMissionSuccess(analysis, userId) {
        ...
        for (const event of timeline) {
            ...
            if (goalLower.includes('punish') && (event.event_type === 'punish_missed' || event.event_type === 'whiff_punish'))
                successCount++;
            if (goalLower.includes('combo') && event.event_type === 'pro_move')
                successCount++;
            if (goalLower.includes('defense') && event.event_type === 'neutral_win')
                successCount++;
            if (goalLower.includes('counter') && descLower.includes('counter'))
                successCount++;
        }
     ...
     */
    static determineSkillUp(goal) {
        const g = goal.toLowerCase();
        if (g.includes('punish') || g.includes('execution') || g.includes('combo'))
            return 'execution';
        if (g.includes('defense') || g.includes('block') || g.includes('tech'))
            return 'defense';
        if (g.includes('neutral') || g.includes('spacing') || g.includes('poking'))
            return 'neutral';
        if (g.includes('knowledge') || g.includes('theory'))
            return 'knowledge';
     ...
     */
    static async applyRewards(userId, xp, skill) {
        ...
        const currentVal = currentStats[skillKey] || 50;
        currentStats[skillKey] = Math.min(100, currentVal + 5);

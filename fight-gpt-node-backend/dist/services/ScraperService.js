...
class ScraperService {
        ...
        try {
            ...
            const roster = await page.evaluate(() => {
                ...
                charLinks.forEach(link => {
                    ...
                    if (!name || name.length < 2 || name.toLowerCase().includes('edit') || name.toLowerCase().includes('file:'))
                        return;
    ...
    async scrapeCombos(characterName) {
        if (!this.browser)
            await this.initialize();
        const context = await this.browser.newContext({
            userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36'
            ...
            const combos = await page.evaluate(() => {
                ...
                tables.forEach(table => {
                    ...
                    for (let i = 1; i < rows.length; i++) {
                        ...
                        if (cols.length <= Math.max(safeInputIdx, safeDamageIdx))
                            continue;
                        const inputStr = cols[safeInputIdx].innerText.trim();
                        const damageStr = cols[safeDamageIdx].innerText.trim();
                        let difficultyStr = 'Beginner';
                        if (difficultyIdx !== -1 && difficultyIdx < cols.length) {
                            difficultyStr = cols[difficultyIdx].innerText.trim();
                        }
                        else if (cols.length >= 6) { // Legacy fallback
                            difficultyStr = cols[5].innerText.trim();
                        }
                        let notesStr = '';
                        if (notesIdx !== -1 && notesIdx < cols.length) {
                            notesStr = cols[notesIdx].innerText.trim();
                        }
                        else if (cols.length >= 7) { // Legacy fallback
                            notesStr = cols[6].innerText.trim();
                        }

...
class SystemInitializer {
    static async run(metaService, rosterSyncService) {
        try {
            ...
            const reportCount = await MetaReport_1.MetaReport.countDocuments();
            // We re-seed if we have 0 or very few reports (force refresh for Deep Intel)
            if (reportCount < 10) {
                logger_1.Logger.info('SYSTEM_INITIALIZATION: Low report density detected. Initializing Deep Proactive Intel 2.0...');
                await this.seedDeepMeta();
                await this.seedDeepTheories();
                await this.seedDeepScenarios();
            }
            else {
                logger_1.Logger.info(`SYSTEM_INITIALIZATION: Detected ${reportCount} meta reports. System state healthy.`);
            }
            ...
            logger_1.Logger.error('SYSTEM_INITIALIZATION: Failed during deep sync sequence', error);
        }
    }
    static async seedCharacterAliases() {
        const ALIASES = {
            // SF6
        ...
        for (const [name, aliases] of Object.entries(ALIASES)) {
            const result = await Character_1.Character.updateMany({ name, aliases: { $size: 0 } }, { $set: { aliases } }).catch(() => ({ modifiedCount: 0 }));
            patched += result.modifiedCount;
        }
        if (patched > 0)
            logger_1.Logger.info(`SYSTEM_INITIALIZATION: Seeded aliases for ${patched} characters`);
    }
    static async ensureAdmin(email) {
        let user = await User_1.default.findOne({ email });
        if (!user) {
            logger_1.Logger.info(`SYSTEM_INITIALIZATION: Creating master admin account: ${email}`);
            ...
            logger_1.Logger.info(`SYSTEM_INITIALIZATION: Promoting ${email} to admin role.`);
            user.role = 'admin';
            await user.save();
        }
    }
    static async seedDeepMeta() {
        const now = new Date();
        ...
        for (const r of reports) {
            await MetaReport_1.MetaReport.findOneAndUpdate({ report_id: r.report_id }, r, { upsert: true });
        }
    }
    static async seedDeepTheories() {
        const theories = [
            {
        ...
        for (const t of theories) {
            await TheoryDocument_1.TheoryDoc.findOneAndUpdate({ theory_id: t.theory_id }, t, { upsert: true });
        }
    }
    static async seedDeepScenarios() {
        ...
        for (const s of scenarios) {
            await Scenario_1.Scenario.findOneAndUpdate({ scenario_id: s.scenario_id }, s, { upsert: true });
        }
    }
}

...
async function updatePriorityCharacters() {
    try {
        logger_js_1.Logger.info('Starting priority character updates...');
        ...
        for (const charName of PRIORITY_CHARACTERS) {
            try {
                const character = await Character_js_1.Character.findOne({ name: charName, game_id: 'sf6', is_current: true });
                if (!character) {
                    logger_js_1.Logger.warn(`${charName} not found`);
                    continue;
                }
                logger_js_1.Logger.info(`Processing ${charName}...`);
                ...
                if (existing) {
                    ...
                    logger_js_1.Logger.info(`✅ Updated ${charName} (${normals.length} normals, ${specials.length} specials)`);
                }
            }
            catch (error) {
                logger_js_1.Logger.error(`Error processing ${charName}:`, error);
            }
        }
        await database_js_1.Database.disconnect();
        logger_js_1.Logger.info('Priority updates complete!');
        process.exit(0);
    }
    catch (error) {
...
//# sourceMappingURL=updatePriorityChars.js.map
...
import { formatGameMetadataForAI, formatCharacterGameRulesForAI } from './aiContextHelper';
...
export class AiPromptHelper {
   ...
   */
  static formatAnalysisContext(
    gameMetadata: IGameMetadata | null,
    characterData: CharacterAnalysisData[]
  ): string {
    const parts: string[] = [];
    ...
    if (characterData.length > 0) {
      ...
      characterData.forEach((char) => {
        ...
        if (char.game_rules && char.game_rules.length > 0) {
          const formattedRules = formatCharacterGameRulesForAI(
            char.game_rules,
            charName
          );
   ...
   */
  private static formatMoveForContext(move: any, compact: boolean = false): string {
    const parts: string[] = [];
   ...
   */
  static formatFullAnalysisContext(
    gameMetadata: IGameMetadata | null,
    p1Character: CharacterAnalysisData | null,
    p2Character: CharacterAnalysisData | null
  ): string {
    const characterData: CharacterAnalysisData[] = [];

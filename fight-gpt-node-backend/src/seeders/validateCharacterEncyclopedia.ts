 ...
 */
function validateDocument(doc: ICharacterEncyclopediaDocument): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Required fields
  if (!doc.game_id) errors.push('Missing game_id');
  if (!doc.character_id) errors.push('Missing character_id');
  if (!doc.patch_version) errors.push('Missing patch_version');
  if (!doc.moveset) errors.push('Missing moveset');
  if (!doc.game_rules) errors.push('Missing game_rules');

  // Validate moveset structure
  if (doc.moveset) {
    if (!Array.isArray(doc.moveset.normals)) errors.push('moveset.normals must be an array');
    if (!Array.isArray(doc.moveset.specials)) errors.push('moveset.specials must be an array');
    if (!Array.isArray(doc.moveset.ex_moves)) errors.push('moveset.ex_moves must be an array');
    if (!Array.isArray(doc.moveset.supers)) errors.push('moveset.supers must be an array');
    ...
    allMoves.forEach((move, index) => {
      ...
      if (!move.input) errors.push(`Move ${index} (${move.name || 'unknown'}) missing input`);
      if (!move.how_to_perform) errors.push(`Move ${index} (${move.name || 'unknown'}) missing how_to_perform`);
      if (!move.category) errors.push(`Move ${index} (${move.name || 'unknown'}) missing category`);
      if (!move.frame_data) {
    ...
    // Warnings
    if (allMoves.length === 0) warnings.push('No moves found in moveset');
    if (doc.moveset.normals.length === 0) warnings.push('No normals found');
    if (doc.moveset.specials.length === 0) warnings.push('No specials found');
  }
  ...
  if (doc.game_rules) {
    if (!Array.isArray(doc.game_rules)) {
      ...
      doc.game_rules.forEach((rule, index) => {
        if (!rule.key) errors.push(`Game rule ${index} missing key`);
        if (rule.value === undefined || rule.value === null) {
          errors.push(`Game rule ${index} (${rule.key || 'unknown'}) missing value`);
        }
        if (!rule.ui_type) errors.push(`Game rule ${index} (${rule.key || 'unknown'}) missing ui_type`);
      });

      if (doc.game_rules.length === 0) warnings.push('No game rules found');
    }
  }
...
async function validateAllDocuments(): Promise<void> {
  try {
    Logger.info('Starting Character Encyclopedia validation...');
    ...
    Logger.info(`Found ${documents.length} documents to validate`);

    if (documents.length === 0) {
      Logger.warn('No documents found to validate');

 ...
 */
const SF6_METADATA: CreateGameMetadataRequest = {
  game_id: 'sf6',
  patch_version: '1.05',
  is_current: true,
  constants: {
 ...
 */
const TEKKEN8_METADATA: CreateGameMetadataRequest = {
  game_id: 'tekken8',
  patch_version: '1.01',
  is_current: true,
  constants: {
 ...
 */
const UMVC3_METADATA: CreateGameMetadataRequest = {
  game_id: 'umvc3',
  patch_version: '1.06',
  is_current: true,
  constants: {
 ...
 */
async function seedGameMetadata(): Promise<void> {
  try {
    Logger.info('Starting Game Metadata seeder...');
    ...
    for (const { name, data } of gameMetadataList) {
      try {
        ...
        const existing = await GameMetadata.findOne({ game_id: data.game_id });
        ...
        if (existing) {
          ...
          if (!existing.is_current || existing.patch_version !== data.patch_version) {
            ...
            await existing.save();
            Logger.info(`✅ Updated game metadata: ${name} (${data.game_id})`);
            updatedCount++;
          } else {
            Logger.warn(`Game metadata for ${name} (${data.game_id}) already exists and is current. Skipping.`);
            skippedCount++;
          }
        } else {
          // Create new metadata
          const gameMetadata = new GameMetadata(data);
          await gameMetadata.save();
          Logger.info(`✅ Created game metadata: ${name} (${data.game_id})`);
          createdCount++;
        }
      } catch (error) {
        Logger.error(`Failed to seed game metadata for ${name}:`, error);
        throw error;
      }
    }
    ...
    Logger.error('Game Metadata seeder error:', error);
    await Database.disconnect();
    process.exit(1);
  }
}

 ...
 */
const SF6_GAME: CreateGameRequest = {
  game_id: 'sf6',
  ...
  description:
    'Street Fighter 6 is the latest entry in the legendary fighting game franchise. Featuring a new RE Engine, Drive System, and World Tour mode.',
  is_active: true,
  latest_version: '1.05',
};
 ...
 */
const SF6_CHARACTERS: CreateCharacterRequest[] = [
  // Ryu
  {
    game_id: 'sf6',
    name: 'Ryu',
 ...
 */
async function seedSF6(): Promise<void> {
  try {
    Logger.info('Starting Street Fighter 6 seeder...');
    ...
    const existingGame = await Game.findOne({ game_id: 'sf6' });
    if (existingGame) {
      Logger.warn('Street Fighter 6 game already exists. Skipping game creation.');
    } else {
      // Create game
      const game = new Game(SF6_GAME);
      await game.save();
      Logger.info(`✅ Created game: ${game.name} (${game.game_id})`);
    }

    // Get the game to use its ID
    const game = await Game.findOne({ game_id: 'sf6' });
    if (!game) {
      throw new Error('Failed to find or create Street Fighter 6 game');
    }

    // Check existing characters
    const existingCharacters = await Character.find({ game_id: 'sf6', is_current: true });
    ...
    for (const characterData of SF6_CHARACTERS) {
      try {
        const character = await Character.findOneAndUpdate(
          { game_id: characterData.game_id, name: characterData.name, version: characterData.version },
          characterData,
          { upsert: true, new: true }
        );
        Logger.info(`✅ Seeded character: ${character.name} (${character.game_id})`);
        createdCount++;
      } catch (error) {
        Logger.error(`Failed to seed character ${characterData.name}:`, error);
        throw error;
      }
    }

    // Update game character count
    const currentCharacters = await Character.find({ game_id: 'sf6', is_current: true });
    game.supported_characters_count = currentCharacters.length;
    await game.save();

    Logger.info(`\n✅ Seeder completed successfully!`);
    Logger.info(`   Game: ${game.name} (${game.game_id})`);
    Logger.info(`   Characters created: ${createdCount}`);
    Logger.info(`   Characters skipped: ${skippedCount}`);
    Logger.info(`   Total characters: ${currentCharacters.length}`);
    Logger.info(`   Game character count updated: ${game.supported_characters_count}`);
  } catch (error) {

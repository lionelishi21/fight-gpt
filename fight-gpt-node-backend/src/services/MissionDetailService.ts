...
export class MissionDetailService {
    ...
    public async getMissionDetails(missionId: string, userId: string): Promise<MissionDetails> {
        ...
        if (!user) throw new Error('User saved data not found');
    ...
    private async populateAntiAirDetails(details: MissionDetails, gameId: string, playerChar: string) {
        // ... existing logic ...
        ...
        if (encyclopedia) {
            const moves = [
                ...
                m.properties?.some(p => p.toLowerCase().includes('anti-air') || p.toLowerCase().includes('aa')) ||
    ...
    private async findVideoReferences(gameId: string, eventType: string): Promise<any[]> {
        ...
        for (const analysis of recent) {
            ...
            const match = timeline.find((t: any) => t.event_type.toLowerCase().includes(eventType) || t.description.toLowerCase().includes(eventType));
    ...
    private async populateParryDetails(details: MissionDetails, gameId: string) {
    ...
    private async populateComboDetails(details: MissionDetails, gameId: string, playerChar: string) {
        const encyclopedia = await this.encyclopediaRepository.findCurrentByGameIdAndCharacterId(gameId, playerChar);
